...
def MAKE_ANSWER(answer, point, dir):
#	print(os.path.exists(dir),file=sys.stderr)
	if os.path.exists(dir) == True:
		#print("tes",file=sys.stderr)
		return False
	else:
		os.mkdir(dir)										#専用のディレクトリを生成
		os.mkdir(dir + "/result")						#結果格納用のディレクトリを生成
		f = open(dir + "/answer.csv", 'w')
...
def MAKE_HTML(part, ques, answer_list, answer, number_of_ques, day, sub, dir):
	#print('make-html',file=sys.stderr)
	f = open("../../../mnt/online_test_2.html", 'w')
	...
	f.write("</head>\n")
	f.write("<header>このシステムはフルスクリーン限定です。また、スクリーンショットなどの不正行為は禁止されています<br> <button id=\"button1\">ページを表示</button></header><body style = \"background-image : url(graPink.gif);\" id = \"target\"><form>")
	...
	#f.write("</script>")
	f.write("</div>")
	f.write("<script src=\"expend.js\"></script>")
	f.write("<script>")
...
def main3(dir):
	"""
	テストを採点用の関数です。
	実装は、格納先のディレクトリを受け取り、ワイルドカードによって記述されたディレクトリに入っているファイルをすべて参照し
	答えようのanswers.csvを中身と比較し、同一かどうか確認。のちにcsvとして書き込みを行う。
	"""
	...
	answers = []
	match = 0			
	with open("../" + dir + "/answer.csv", 'r') as f:
		answer = csv.reader(f)
...
dir = "../../../opt/data/" + receive["sub"] + "_" + receive["day"]	
print(dir)
#print(receive,file=sys.stderr)
"""
if MAKE_ANSWER(receive["answer"], receive["point"], dir) == True:
	#MAKE_HTML(receive["part"], receive["ques"], receive["answer_list"], receive["answer"], int(receive["number_of_ques"]), receive["day"], receive["sub"], dir)
	print(1)
else:
